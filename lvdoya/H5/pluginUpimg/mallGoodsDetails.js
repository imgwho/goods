...
var config = {
    ...
    'server_url':"http://www.lvdoya.com/app/imgUploadPHP/upImg_mallGoodsFace.php",//提交数据远程服务器地址
    "post_btn_class":".submit_data",//你页面上表单按钮的class, 可以修改成你自己的
    "progress":'#fb_load',
    "uploadid":"#tb_upMallGoodsDetails"//上传控件的元素id
...
/*    for ( var i in that.files){
	...
	var imgChoosed=parseInt($("#font_imgNo").html());
    for ( var i in that.files){
        if(!isNaN(i) && i<9-imgChoosed){
            _compress(that.files[i],options);
        }
		if (i==that.files.length-1){
			setTimeout(function(){$("#loadingToast").css("display","none");},500);	//处理完成后添加延迟执行
		} 
    } 	
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.status==1){
				...
				$("#hid_goodsDetails").val($("#hid_goodsDetails").val()+"<img src='http://www.lvdoya.com/app/imgGoods/"+filename+"'/>,");
				//alert($("#hid_goodsDetails").val());
            }else{
...
function post_mallGoodsDetails(){
